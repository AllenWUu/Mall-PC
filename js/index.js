...
window.onload = function () {
  ...
  function thumbnailClick() {
    ...
    for (var i = 0; i < liNodes.length; i++) {
      ...
      liNodes[i].onclick = function () {
        var idx =
          this.index /** 事件函数中的this永远指向的是实际发生事件的目标源对象 */
        bigimgIndex = idx
  ...
  function thumbnailLeftRightClick() {
    ...
    let temp = 0
    //(2) 计算每一次所走过的步长
    let everyLeft = (liNodes[0].offsetWidth + 20) * 2
    //(3) 计算ul所可以移动的总距离 = ul的宽度 - div框的宽度 = （图片总数-图片显示数）*一个图片的宽度
    let sumLeft = (liNodes.length - 5) * (liNodes[0].offsetWidth + 20)
    console.log(sumLeft)
    let prev = document.querySelector(
      '#wrapper #content .contentMain #center #left #leftBottom .prev'
    )
    prev.onclick = function () {
      temp += everyLeft
      if (temp > sumLeft) temp = sumLeft
      ul.style.left = -temp + 'px'
      console.log('prev');
      console.log(ul);
    }
    let next = document.querySelector(
      '#wrapper #content .contentMain #center #left #leftBottom .next'
    )
